...
interface DataContextType extends DataState {
    ...
    getProductsByCompany: (companyId: string | number) => ProductWithLatestWarranty[];
    isLoaded: boolean;
}
...
export function DataProvider({ children }: { children: React.ReactNode }) {
    ...
    const getCompany = useCallback((id: string | number) => {
        return state.companies.find(c => String(c.id) === String(id));
    }, [state.companies]);

    const getProductsByCompany = useCallback((companyId: string | number) => {
        return state.products.filter(p => String(p.companyId) === String(companyId));
    }, [state.products]);
