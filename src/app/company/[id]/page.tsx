...
import { Package, Calendar, User, Hash, ArrowLeft, Edit, Clock, MapPin } from "lucide-react";
                ...
                <Card className="lg:col-span-1 h-fit">
                    <CardHeader className="flex flex-row items-start justify-between space-y-0">
                        ...
                        <Link href={`/company/edit/${companyId}`}>
                            <Button variant="outline" size="sm" className="gap-2 h-8 px-2 text-xs">
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-1">
                            <span className="text-xs font-bold uppercase text-muted-foreground">เลขประจำตัวผู้เสียภาษี</span>
                            <p className="text-sm font-medium">{company.taxId || "ไม่ได้ระบุ"}</p>
                        </div>
                        <div className="space-y-1">
                            <span className="text-xs font-bold uppercase text-muted-foreground">ข้อมูลการติดต่อ</span>
                            <p className="text-sm border p-3 rounded-lg bg-slate-50 min-h-[100px] whitespace-pre-wrap">{company.contactInfo || "ไม่มีข้อมูลการติดต่อ"}</p>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <div className="grid gap-4">
                        ...
                        {filteredProducts.length === 0 ? (
                            ...
                            filteredProducts.map((product: any) => {
                                ...
                                const isActive = activeW && new Date(activeW.startDate) <= now && new Date(activeW.endDate) >= now;
                                const isExpired = activeW && new Date(activeW.endDate) < now;
                                const isUpcoming = activeW && new Date(activeW.startDate) > now;
                                const isExpiringSoon = isActive && new Date(activeW.endDate) < oneMonthFromNow;

                                const statusColor = isExpired ? '#ef4444' :
                                    isUpcoming ? '#3b82f6' :
                                        isExpiringSoon ? '#f59e0b' : // Orange
                                            isActive ? '#22c55e' : '#94a3b8';

                                return (
                                    <Link key={product.id} href={`/product/${product.id}`}>
                                        <Card className="hover:border-primary transition-all group cursor-pointer overflow-hidden border-l-4"
                                            ...
                                            <CardContent className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                        ...
                                                        isExpired ? "bg-red-100 text-red-600" :
                                                            isUpcoming ? "bg-blue-100 text-blue-600" :
                                                                isExpiringSoon ? "bg-amber-100 text-amber-600" :
                                                                    isActive ? "bg-green-100 text-green-600" : "bg-primary/10 text-primary"
                                                    )}>
                                                            ...
                                                            {activeW && (
                                                                <span className={cn(
                                                                    ...
                                                                    isExpired ? "bg-red-50 text-red-600 border-red-100" :
                                                                        isUpcoming ? "bg-blue-50 text-blue-700 border-blue-100" :
                                                                            isExpiringSoon ? "bg-amber-50 text-amber-700 border-amber-200" :
                                                                                "bg-green-100 text-green-700 border-green-200"
                                                                )}>
                                                                    {isExpired ? `หมดประกัน: ${activeW.type}` :
                                                                        isUpcoming ? `รอเริ่ม: ${activeW.type}` :
                                                                            isExpiringSoon ? `ใกล้หมดประกัน: ${activeW.type}` :
                                                                                `ยังไม่หมดประกัน: ${activeW.type}`}
                                                                </span>
                                                            )}
                                                        </div>
                                                        <div className="flex gap-4 text-xs text-muted-foreground mt-1">
                                                            <span className="flex items-center gap-1"><Hash className="w-3 h-3" /> {product.serialNumber}</span>
                                                            {activeW && (
                                                                <span className="flex items-center gap-2 text-slate-500 font-medium">
                                                                    <Clock className="w-3 h-3 text-primary/60" />
                                                                    <span className="flex items-center gap-1">
                                                                        {new Date(activeW.startDate).toLocaleDateString()}
                                                                        <span className="mx-1 opacity-50">ถึง</span>
                                                                        {new Date(activeW.endDate).toLocaleDateString()}
                                                                    </span>
                                                                </span>
                                                            )}
                                                        </div>
                                                ...
                                                <div className="flex items-center gap-6 text-sm text-muted-foreground">
                                                    {product.branch && (
                                                        <div className="flex items-center gap-2" title="สาขา">
                                                            <MapPin className="w-4 h-4 text-primary/40" />
                                                            {product.branch}
                                                        </div>
                                                    )}
                                                    <div className="flex items-center gap-2" title="วันที่ซื้อ">
                                                        <Calendar className="w-4 h-4 text-primary/40" />
                                                        {product.purchaseDate ? new Date(product.purchaseDate).toLocaleDateString() : "ไม่ได้ระบุ"}
                                                    </div>
                                                    <div className="flex items-center gap-2" title="ผู้ติดต่อ">
